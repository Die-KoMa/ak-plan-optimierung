    ...
    with open(csv_input_file_name, newline="") as csv_file:
        ...
        for row in reader:
            if reader.line_num == 1:
                ...
                person_dict[name] = {"name": name, "attend": attend, "lead": lead}
...
def solve(room_dict, slot_dict, ak_dict, person_dict):
    ...
    model.addConstrs((x.sum("*", "*", a) == 1 for a in ak_dict), "ak_stattfinden")

    # AK-Leitung darf sich nicht ueberschneiden:
    for person in person_dict.values():
        for s, slot in slot_dict.items():
            model.addConstr(
                sum(x[s, r, a] for r in slot["rooms"] for a in person["lead"]) <= 1,
    ...
    for s, r in available:
        model.addConstr(
            sum(x[s, r, a] for person in person_dict.values() for a in person["attend"])
            <= room_dict[r]["capacity"],
    ...
    rel_coeffs = {
        (s, p): 1 / len(person["attend"]) if len(person["attend"]) > 0 else 0
        for s in slot_dict
        for p, person in person_dict.items()
    }
    ...
    for s, slot in slot_dict.items():
        for p, person in person_dict.items():
            model.addConstr(
                sum(x[s, r, a] for r in slot["rooms"] for a in person["attend"])
                - overlap[s, p]
                <= 1,
    ...
    model.addConstr(
        overlap.prod(rel_coeffs) - sum_rel_overlap == 0, "summe rel_uberschneidung"
    )

    # Set objective
    #    model.setObjective( sum_rel_overlap + 1*len(person_dict)*max_rel_overlap , GRB.MINIMIZE )
    model.setObjective(
        sum_rel_overlap + 1 / 2 * len(person_dict) * max_rel_overlap, GRB.MINIMIZE
    )
...
def _main():
    ...
    room_dict, slot_dict, ak_dict, person_dict = read(
        json_input_file_name, csv_input_file_name
    )
